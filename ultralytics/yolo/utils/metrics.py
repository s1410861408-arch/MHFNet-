...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, EIoU=False, SIoU=False, Shape_IOU=False, eps=1e-7):
    """
    Calculate Intersection over Union (IoU) of box1(1, 4) to box2(n, 4).
    ...
    if Shape_IOU:
        ...
        # NWD-Shape
        B = ((torch.abs(w1 - w2)) ** 2 + (torch.abs(h1 - h2)) ** 2)/4
        D = torch.sqrt(center_distance + B)
        shape_cost = torch.exp(-D/C)
        # Shape-IoU
