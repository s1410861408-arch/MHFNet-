...
def model_info(model, detailed=False, verbose=True, imgsz=640):
    ...
    LOGGER.info(f'{model_name} summary{fused}: {n_l} layers, {n_p} parameters, {n_g} gradients, {fs}, {flops} flops')
...
def get_flops(model, imgsz=640):
    ...
    try:
        ...
        try:
            # 确定模型期望的输入通道数
            input_channels = 3  # 默认情况下，我们假设输入为RGB图像，通道数为3
            # 如果模型具有自定义的输入通道数，可以通过检查模型的第一层来获取
...
def init_seeds(seed=0, deterministic=False):
    ...
    if deterministic:  # https://github.com/ultralytics/yolov5/pull/8213
        if TORCH_1_12:
            ...
            LOGGER.warning('WARNING ⚠️ Upgrade to torch>=1.12.0 for deterministic training.')
